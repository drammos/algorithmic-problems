...
Διορθωσεις απο Δευτερη εργασια:

    Στην δευτερη εργασια ειχαμε 2 λαθη
        1. 
            Το πρωτο αφορα το εμβαδον του πολυγωνου απο convex_hull που εβγαινε παντα αρνητικο.
            Αυτο οφειλοταν στο γεγονος οτι ο convex_hull επεστρεφε το πολυγωνο με την φορα του ρολογιου, ομως το διορθωσαμε
            και το εμβαδον του πολυγωνου που επιστρεφεται ειναι παντα θετικο.
        2.
            Το δευτερο λαθος ηταν στο Subdivision, δεν ηταν σωστο το τελικο αποτελεσμα του πολυγωνου καποιες φορες.
            Το λαθος αυτο οφειλεται στο γεγονος οτι στο subdivision μαρκαραμε συγκεκριμενες ακμες για καθε spal-πολυγωνο ωστε
            να μην τις τροποποιηση ο convex_hull. Ομως επειδη ο convex_hull στην αρχη ελεγχει αν αναμεσα στο αρχικο πολυγωνο που δημιουργειται
            αν καποια απο τις ακμες του πχ η ακμη Α-Γ εχει αναμεσα της σημειο Β το οποιο εφαπτεται με την ακμη αυτη και το θεωρει ως εσωτερικο σημειο 
            και οχι ως μερο του πολυγωνου, τοτε δημιορυγει 2 ακμες την A-B KAI B-Γ.

            Επομενως αν καποιες φορες ο αλγοριθμος μας Subdivision μαρκαρε την A-Γ ο convex_hull την τροποιουσε. Επομενως η αλλαγησ
            στο σωμα του subdivision ειναι οτι ελεγχουμε εκει για καθε spal-polygon για τετοια σημεια και επειτα μαρκαρουμε τις ακμες που 
            δεν θελουμε να αλλαξει ο convex_hull. Επομενως το αποτελεσμα ειναι παντα σωστο για καθε εισοδο.

            Στο convex_hull για να μην γινεται διπλη φορα η διαδικασια αν καλειται απο το subdivision ο παραπανως ελεγχος υλοποιειται σε ενα 
            function set_up που καλειται μονο αν η κλιση του function προερχεται απο καποιο αλλο αλγοριθμο εκτος απο αυτον του subdivision.    
        3.
            Στο global step του simulated annealing έγινε μία βελτιστοποίηση όσον αφορά τον τελικό έλεγχο τομής των νέων ακμών. Αυτό βοήθησε
            ώστε ο συγκεκριμένος αλγόριθμος να κόβεται λίγο πιο σπάνια από το cut-off.
...
Size || min score | max score | min bound | max bound || min score | max score | min bound | max bound || min score | max score | min bound | max bound || min score | max score | min bound | max bound || min score | max score | min bound | max bound || min score | max score | min bound | max bound || min score | max score | min bound | max bound ||
 ...
 100 || 1.00000   |  0.00000  |  1.00000  |  0.00000  ||  1.00000  |  0.00000  |  1.00000  |  0.00000  ||  0.55995  |  0.52099  |  0.55995  |  0.52099  ||  0.53379  |  0.51556  |  0.53379  |  0.51556  ||  0.59748  |  0.52014  |  0.59748  |  0.52014  ||  1.00000  |  0.53105  |  1.00000  |  0.53105  ||
...
2000 || 1.00000   |  0.00000  |  1.00000  |  0.00000  ||  1.00000  |  0.00000  |  1.00000  |  0.00000  ||  0.41584  |  0.41691  |  0.41584  |  0.41691  ||  0.46159  |  0.50164  |  0.46159  |  0.50164  ||  1.00000  |  0.00000  |  1.00000  |  0.00000  ||  1.00000  |  0.00000  |  1.00000  |  0.00000  ||  1.00000  |  0.00000  |  1.00000  |  0.00000  ||

Προφανώς παρατηρούμε ότι για μεγαλύτερες εισόδου πολλοί αλγόριθμοι αργούν και ξεπερνούν χρονικά το cutoff. Όπως προαναφέραμε προσπαθήσαμε να
κάνουμε κάποιες χρονικές βελτιστοποιήσεις όμως ακόμα οι αλγόριθμοι τρέχουν σχετικά αργά.

Για τον local search αυτό δεν μας εκπλήσσει καθώς πρόκειται για έναν εξαιρετικά αργό αλγόριθμο.

Όσον αφορά τον simulated annealing με local step, αυτός συνήθως δεν ξεπερνάει το όριο και δίνει σχετικά γρήγορα αποτελέσματα.
Βέβαια, όπως είχαμε αναφέρει και στην προηγούμενη εργασία, επειδή ο simulated annealing δέχεται και κακές ενδιάμεσες λύσεις,
τυχαίναι καμία φορά ο αλγοριθμος αυτός να μην δίνει τόσο καλα ratio.

Το ίδιο εννοείται συμβαίνει και στην περίπτωση του global step.
Μάλιστα, ο global step, όπως βλέπουμε στο παράδειγμα με  τα 200 σημεία, προλαβαίνει να δώσει αποτέλεσμα για το max αλλα όχι για το min.
Φαίνεται ότι στην περίπτωση του min ο υπολογισμός της ενέργειας ίσως προκαλεί περισσότερες επαναλήψεις.

Τέλος, δοκιμάσαμε να τρέξουμε τον subdivision για 2000 σημεία αλλά δυστυχώς δεν πρόλαβε να δώσει αποτελέσματα.

2000 ήταν και το μέγιστο που δοκιμάσαμε καθώς η εκτέλεση ήταν εξαιρετικά αργη.